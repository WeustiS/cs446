...
class UNet(nn.Module):
    def __init__(self, in_dim, out_dim, num_filters):
        ...
        # Bridge
        self.bridge = unet_conv_block(self.num_filters * 16, self.num_filters * 16)
        self.bridge2 = unet_conv_block(self.num_filters * 16, self.num_filters * 32)
        # Up sampling
        ...
        self.one = nn.Conv3d(out_dim, out_dim, kernel_size=1, stride=1, padding=0)
    ...
    def forward(self, x):
        ...
        # Bridge
        bridge_1 = self.bridge(pool_5)  # -> [n, 128, 6, 6, 6]
        bridge_2 = self.bridge2(bridge_1)

        # Up sampling
        trans_1 = self.trans_1(bridge_2)  # -> [n, 128, 12, 12, 12]
        ...
        cat_2 = torch.cat([trans_2, down_4], dim=1)  # -> [n, (64+32),24,24,24]
        up_2 = self.up_2(cat_2)  # -> [n, 32, 24,24,24]

        trans_3 = self.trans_3(up_2)  # -> [n, 32, 48, 48, 48]
        cat_3 = torch.cat([trans_3, down_3], dim=1)  # -> [n, (32+16), 48, 48, 48]
        up_3 = self.up_3(cat_3)  # -> [n, 16, 48, 48, 48]

        trans_4 = self.trans_4(up_3)  # -> [n, 16, 96, 96, 96]
        cat_4 = torch.cat([trans_4, down_2], dim=1)  # -> [n, (16+8), 96, 96, 96]
        up_4 = self.up_4(cat_4)  # -> [n, 8, 96, 96, 96]

        trans_5 = self.trans_5(up_4)  # -> [n, 8, 192, 192, 192]
        cat_5 = torch.cat([trans_5, down_1], dim=1)  # -> [n, (8+4), 192, 192, 192]
        up_5 = self.up_5(cat_5)  # -> [n, 4, 192, 192, 192]

        # Output
        out = self.out(up_5)  # -> [n, out, 192, 192, 192]
        out_one = self.one(out) # 1x1 conv [n, out, 192, 192, 192]
        o = torch.softmax(out_one, dim=1) # -> [n, out(softmax), 192,192,192]
